...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`tempfile`](https://crates.io/crates/tempfile).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `tempfile` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Temporary file / dir / handle utilities
...
Original work copyrighted by the upstream `tempfile` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
