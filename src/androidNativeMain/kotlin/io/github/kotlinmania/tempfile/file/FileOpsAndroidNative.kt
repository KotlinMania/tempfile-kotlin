...
internal actual fun createNamedFile(path: String): Result<Unit> {
    val fd = open(path, O_RDWR or O_CREAT or O_EXCL, 0b110_000_000)
    if (fd >= 0) {
        close(fd)
    ...
    return Result
        .failure<Unit>(
            IoException(posixErrnoToKind(errno), "create file failed: errno=$errno"),
        ).withErrPath { path }
}
...
internal actual fun removeFile(path: String): Result<Unit> {
    ...
    return Result
        .failure<Unit>(
            IoException(posixErrnoToKind(errno), "remove file failed: errno=$errno"),
        ).withErrPath { path }
}
...
internal actual fun persistFile(oldPath: String, newPath: String, overwrite: Boolean): Result<Unit> {
    if (overwrite) {
        ...
        return Result
            .failure<Unit>(
                IoException(posixErrnoToKind(errno), "rename failed: errno=$errno"),
            ).withErrPath { oldPath }
    } else {
        ...
        return Result
            .failure<Unit>(
                IoException(posixErrnoToKind(errno), "link failed: errno=$errno"),
            ).withErrPath { oldPath }
    }
}
...
internal actual fun readBytes(path: String): Result<ByteArray> {
    ...
    if (fp == null) {
        return Result
            .failure<ByteArray>(
                IoException(posixErrnoToKind(errno), "fopen failed: errno=$errno"),
            ).withErrPath { path }
    }
    try {
