# Claude Code Project Instructions — tempfile-kotlin

## Project Overview

This is **tempfile-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tempfile`](https://crates.io/crates/tempfile).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tempfile/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tempfile>
